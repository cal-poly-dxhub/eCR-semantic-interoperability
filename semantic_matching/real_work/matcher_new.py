...
class DocumentSchemaMapper:
    ...
    def _generate_document_embeddings(self, doc_items: List[Dict[str, str]]) -> None:
        ...
        for item in tqdm(doc_items, desc="Preparing items"):
            path = item['path']
            value = item['value']
        ...
        for i, item in tqdm(enumerate(items), desc="Storing embeddings", total=len(items)):
    ...
    def _find_schema_matches(self, top_k: int = 10) -> Dict[str, List[Tuple[str, float]]]:
        ...
        for field_path, field_data in tqdm(self.schema_embeddings.items(), desc="Processing schema fields"):
            field_matches = []
