...
def create_patient(given_name, gender_code, date, marital_status, mobile_phone):
  ...
  return(Patient( extension = [], telecom=[ContactPointType( use=CodeType('mobile'), value=mobile_phone)],birthDate=date, gender=gender_code, name=[HumanName(given=[given_name])]))
...
def fill_in_patient(XML):
  prompt = fill_in_patient_prompt.replace('--XML--',XML).replace('--PATIENT--',objects['Patient'].json())
